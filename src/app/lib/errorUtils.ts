  ...
  if (process.env.NODE_ENV === 'development') {
    switch (resultCode) {
      ...
      case '30':
        return {
          title: '일시적인 서비스 오류',
          message: '등록되지 않은 서비스 키입니다. 서비스 설정을 확인해주세요.',
        };
    ...
    switch (resultCode) {
      ...
      case '05': // SERVICETIME_OUT: 서비스 연결실패 에러
      case '20': // SERVICE_ACCESS_DENIED_ERROR: 서비스 접근거부
      case '21': // TEMPORARILY_DISABLE_THE_SERVICEKEY_ERROR: 일시적으로 사용할 수 없는 서비스 키
        return {
          title: '일시적인 서비스 오류',
          message:
            '현재 날씨 정보를 불러오는 데 문제가 발생했습니다. 잠시 후 다시 시도해주세요.',
      ...
      case '11': // NO_MANDATORY_REQUEST_PARAMETERS_ERROR: 필수요청 파라미터가 없음
      case '12': // NO_OPENAPI_SERVICE_ERROR: 해당 오픈API서비스가 없거나 폐기됨
      case '22': // LIMITED_NUMBER_OF_SERVICE_REQUESTS_EXCEEDS_ERROR: 서비스 요청제한횟수 초과에러
      case '30': // SERVICE_KEY_IS_NOT_REGISTERED_ERROR: 등록되지 않은 서비스키
      case '31': // DEADLINE_HAS_EXPIRED_ERROR: 기한만료된 서비스키
        return {
          title: '서비스 오류',
          message:
            '서비스에 문제가 발생하여 현재 날씨 정보를 제공할 수 없습니다. 빠른 시일 내에 해결하도록 하겠습니다.',
