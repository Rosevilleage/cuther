...
type Coordinate = {lat: number; lng: number; nx: number; ny: number};

export function getXYConv(v1: number, v2: number): Coordinate {
  ...
  theta *= sn;
  const nx: number = Math.floor(ra * Math.sin(theta) + XO + 0.5);
  const ny: number = Math.floor(ro - ra * Math.cos(theta) + YO + 0.5);
  const xn: number = nx - XO;
  const yn: number = ro - ny + YO;

  const alat: number =
    2.0 *
      Math.atan(Math.pow((re * sf) / Math.sqrt(xn * xn + yn * yn), 1.0 / sn)) -
    Math.PI * 0.5;
  const alon: number =
    Math.abs(xn) <= 0.0 ? 0.0 : Math.atan2(xn, yn) / sn + olon;
  return {
    lat: alat * RADDEG,
    lng: alon * RADDEG,
    nx,
